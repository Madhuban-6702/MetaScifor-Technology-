...
def register(request):
    if request.method == 'POST':
        ...
        # Check if phone number already exists (use phone field, not username)
        if User.objects.filter(phone=phone).exists():  # Checking in phone field
        ...
        user = User.objects.create_user(
            username=username,  # You can use email or some other field as the username
            first_name=first_name,
            last_name=last_name,
            email=email,
            password=password,
            phone=phone,  # Store the phone number separately
        )
        user.save()
...
def edit_profile(request):
    if request.method == 'POST':
        ...
        return redirect('profile')  # Adjust to the URL name of your profile page
